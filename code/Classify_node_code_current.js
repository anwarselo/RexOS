...
function classifyMessage(standardizedData) {
  ...
  const messageContent = (standardizedData.messageContent || "").toLowerCase();
  
  // If this is not a text message, we can't classify it using text patterns
  if (standardizedData.messageType && standardizedData.messageType !== "text") {
    return {
      ...standardizedData,
      classification: {
        type: "unclassifiable",
        confidence: 0.9,
        reason: `Non-text message type: ${standardizedData.messageType}`
      }
    };
  }
